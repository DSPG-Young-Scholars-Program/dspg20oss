...
# Email domain
Match email address country domain
```{r}
...
gh_extra_company$company <- str_replace_all(gh_extra_company$company , fixed("@"), "") 
...
company_confirm_gov_vector <- as.data.frame(unlist(company_confirm_gov$company))
colnames(company_confirm_gov_vector) <- "company"
company_confirm_gov_vector <- company_confirm_gov_vector%>%
   mutate(company2 = str_replace_all(company , fixed("u.s."), "united states"))%>%
  mutate(company3 = str_replace_all(company , fixed("u.s."), "us"))%>%
   gather("company_type",  "company_name")%>%
   select(-company_type)%>%
...
```{r}
bag_of_words <- gh_extra_company%>%
  filter(is.gov)%>%
  select(company)%>%
  filter(!is.na(company))

bag_of_words <- as.data.frame(unlist(strsplit(bag_of_words$company, "\\ ")))
