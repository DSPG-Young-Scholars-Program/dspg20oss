...
institutions_hipolabs <- dbGetQuery(conn, "SELECT institution, country, country_code, domains FROM hipolabs.universities")
```
...
sum(initial_overlap[,"n"]) # 16,443 identified users from academic institutions
```
...
At this point, we have 23,717 users matched to academic institutions. 
...
New total is 35,405 users matched to academic institutions.

```{r}
# use hipolabs dataset to get countries
academic_counts <- left_join(overlap, distinct(institutions_hipolabs, institution, country, country_code), by = c("organization" = "institution"))

sum(academic_counts[,"n"]) # 36054

# larger than before because it's double-counting the institutions that are in multiple countries
academic_counts[duplicated(academic_counts$organization), ]
df <- academic_counts[!duplicated(academic_counts$organization), ] # will need to fine-tune this to select the primary country

sum(df[,"n"]) # 35,405

# everyone likes visuals
ggplot(data = df[1:10,]) + geom_col(mapping = aes(n, reorder(organization, n), fill = n)) + labs(y = "Academic institution", x = "User count")
```
