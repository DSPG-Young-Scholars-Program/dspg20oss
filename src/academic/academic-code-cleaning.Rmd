...
users_gh <- dbGetQuery(conn, "SELECT login, company
                              FROM gh.ctrs_raw")

institutions_table <- dbGetQuery(conn, "SELECT institution, country_code FROM hipolabs.universities")
```
...
org_counts <- users_gh %>%
  ...
  dt_mutate(organization = ifelse(test = str_detect(string = organization, 
                              pattern = "\\b(?i)(university of illinois at urbana-champaign|university of illinois urbana-champaign|university of illinois, urbana-champaign|university of illinois at urbana champaign)\\b"), 
  ...
  dt_mutate(organization = ifelse(test = str_detect(string = organization, 
                              pattern = "\\b(?i)(university of colorado boulder|university of colorado, boulder|university of colorado - boulder|university of colorado-boulder)\\b"),
...
Check how many academic institutions are represented by comparing with hipolabs data

```{r}
institutions_table <- as.data.table(institutions_table)
dt_mutate(institutions_table, institution = str_to_lower(institution))
overlap <- semi_join(org_counts, institutions_table, by = c("organization" = "institution"))
overlap
```
...
Institutions that are either missing in hipolabs data, or the user reported a broad instituion where hipolabs has only specific locations named:

university of michigan, sun yat-sen university, texas a&m university, university of college london, university of tokyo, university of texas, university of maryland, xidian university, ohio state university, broad institute, university of illinois, iiit hyderabad (exists twice and is wrongly listed as "indian institute of technology, hyderabad" when it should be "international institute of technology, hyderabad"), beihang university
