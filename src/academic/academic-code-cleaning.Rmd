...
sum(initial_overlap[,"n"]) # 17,128 identified users from academic institutions
```
...
At this point, we have 29,115 users in the academic sector: 27,551 matched to academic institutions and 1,564 identified as misc. students.
...
nrow(imputed_institutions) # 11,761 imputed
...
New total is 40,876 users in the academic sector.
...
test <- academic_counts[duplicated(academic_counts$organization), ] %>% rename(c(country2 = country, domains2 = domains))
test2 <- test[duplicated(test$organization), ] %>% rename(c(country3 = country2, domains3 = domains2))
test3 <- test2[duplicated(test2$organization), ] %>% rename(c(country4 = country3, domains4 = domains3))
test4 <- test3[duplicated(test3$organization), ] %>% rename(c(country5 = country4, domains5 = domains4))

test <- test[!duplicated(test$organization), ]
test2 <- test2[!duplicated(test2$organization), ]
test3 <- test3[!duplicated(test3$organization), ]
test4 <- test4[!duplicated(test4$organization), ]
...
academic_counts_sorted <- academic_counts %>% 
  dt_mutate(n = as.numeric(n)) %>%
  arrange(n) %>%
  dt_mutate(organization = factor(organization, levels = organization))

ggplot(data = academic_counts_sorted[(.N-10):.N,]) + geom_col(mapping = aes(organization, n, fill = n)) + labs(x = "Academic institution", y = "User count") + coord_flip()
```

```{r}
# get some good numbers
percent_us_institution <- length(filter(academic_counts, country == "United States")$organization) / length(academic_counts$organization) # 0.330
percent_us_users <- sum(filter(academic_counts, country == "United States")[, "n"]) / sum(academic_counts[, "n"]) # 0.543
percent_academic_users <- sum(academic_counts[,"n"]) / 2435698 # 0.016
