...
users_gh %>% 
  ...
  dt_mutate(nonprofits = ifelse(test = str_detect(string = company, 
            pattern = charities_vector), yes = "charities", no = NA)) %>%
  dt_mutate(nonprofits = ifelse(test = str_detect(string = company, 
            pattern = ngo_vector), yes = "ngos", no = nonprofits)) %>%
  dt_mutate(nonprofits = ifelse(test = str_detect(string = company, 
           pattern = govadmins_vector), yes = "admins", no = nonprofits)) %>%
  drop_na(nonprofits) %>% 
  select(login, company, nonprofits)
