...
city_nodelist <- dbGetQuery(conn, "SELECT * FROM gh.sna_ctr_city_codes")
...
unique(city_nodelist$c_continent_name)

# north america network
intl_ctr_nodelist <- city_nodelist%>% 
  filter(c_continent_name == "North America")
...
intl_ctr_edgelist <- ctr_edgelist %>% 
  ...
  #filter(ctr1 != ctr2) %>% 
  rename(login = ctr1) %>% 
  inner_join(intl_ctr_nodelist, by = c( "login" = "raw_login")) %>% 
