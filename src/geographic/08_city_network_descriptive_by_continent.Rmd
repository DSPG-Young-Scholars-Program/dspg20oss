...
for (pkg in c("tidyverse", "data.table", "R.utils", "RPostgreSQL", 
              ...
              "cowplot", "maditr", "lubridate", "countrycode", "tools")) {library(pkg, character.only = TRUE)}
...
# city_edgelist<- dbGetQuery(conn, "SELECT * FROM gh.sna_city_northamerica_edgelist_0819")
...
city_edgelist_us <- city_edgelist%>%
...
write.csv(city_edgelist_us, file = "sna_city_us_edgelist_0819.csv", row.names = F)

#number of unique sources
source <- city_edgelist_us$source
source <- unique(source)
length(source)

#number of unique targets
target <- city_edgelist_us$target
target <- unique(target)
length(target)

#construct nodelist for viz on gephi
city_nodelist_us <- c(source, target)
length(city_nodelist_us)
city_nodelist_us <- unique(city_nodelist_us)
length(city_nodelist_us)
...
city_nodelist <- ctrs_city%>%
  select(c_city_code, raw_country_code, raw_city, c_country_name)%>%
  filter(c_city_code %in% city_nodelist_us)%>%
  mutate(raw_city = toTitleCase(raw_city))%>%
  rename(Id = c_city_code, country = raw_country_code, city = raw_city)

table(city_nodelist$c_country_name)

write.csv(city_nodelist, file = "sna_city_us_nodelist_0819.csv", row.names = F)
```
