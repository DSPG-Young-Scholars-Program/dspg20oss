...
city_edgelist_us <- city_edgelist%>%
  mutate(target_is_us = if_else(str_detect(target, "us"), T, F))%>%
  mutate(source_is_us = if_else( str_detect(source, "us"), T, F))%>%
...
city_nodelist_us <- c(city_edgelist_us$source, city_edgelist_us$target)
city_nodelist_us <- unique(city_nodelist_us)

city_nodelist_us <-  city_nodelist_us%>%
  as.data.frame()%>%
  rename(city_code = ".")%>%
  mutate(country = substr(city_code, start = 1, stop = 2) )
#write.csv(city_nodelist_us, file = "sna_city_us_nodelist_0819.csv", row.names = F)
