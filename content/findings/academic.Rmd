...
```{r, by_country, fig.height = 9, fig.width=12, warning = FALSE,  echo=FALSE}
<<<<<<< HEAD
...
=======
academic_counts_sorted2 <- academic_counts_sorted[(.N-20):.N, ]
#academic_counts_sorted2 <- academic_counts_sorted %>% arrange(-n) %>% slice(1:20)

#academic_counts_sorted2 <- academic_counts_sorted2 %>% 
#  mutate(organization = as.character(organization)) %>% 
#  mutate(organization = ifelse(test = str_detect(string = organization, 
#                       pattern = "university of california, berkeley"), 
#                       yes = "uc berkeley", no = organization)) %>% 
#  mutate(organization = ifelse(test = str_detect(string = organization, 
#                       pattern = "university of california, san diego"), 
#                       yes = "uc berkeley", no = organization)) %>% 
#  mutate(organization = ifelse(test = str_detect(string = organization, 
#                       pattern = "university of illinois at urbana-champaign"), 
#                       yes = "univ of illinois, uc", no = organization)) %>% 
#  mutate(organization = ifelse(test = str_detect(string = organization, 
#                       pattern = "university of michigan - ann arbor"), 
#                       yes = "univ of michigan", no = organization)) %>% 
#  mutate(organization = as.factor(organization)) 
  

academic_counts_sorted2 %>% 
  ggplot() + geom_point(aes(x = organization, y = n, color = country), size = 4) +
  geom_segment(data = academic_counts_sorted[(.N-20):.N, ], 
               mapping = aes(x = organization, xend = organization, y = 0, yend = n), size = 1) +
  scale_color_manual(labels = c("canada", "china", "united states"), 
>>>>>>> 0d111336ca25c43b757a32cd98a47444d39091ff
...
```{r, us_universities, fig.height = 9, fig.width = 12,  warning = FALSE, echo=FALSE}
