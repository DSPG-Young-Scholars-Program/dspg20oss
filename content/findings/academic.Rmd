...
First, we looked for a data source for organizations in the academic sector. We ended up using [this dataset](https://github.com/Hipo/university-domains-list/) from Hipo Labs, which has the names, countries, and domain names for most universities in the world. (how many?) With this, we could identify which users are in the academic sector by looking at what self-reported company they have on their profile, and looking for a match in our university dataset. 

However, only around 20% of our GitHub users have any company information on their profile, and those who did had staggering amounts of variation in how they reported their company. (examples?) 
...
ggplot(country_users_sorted[(.N-20):.N, ]) + #, aes(color = country)
