...
ggplot(gh_country, aes(x = c_country_name, y = ttl_users, fill = ttl_users)) +
  geom_bar(stat = "identity", fill = uva_color_palette[9])+
...
ggplot(gh_city, aes(x = city_country, y = ttl_users))+
  geom_bar(stat = "identity", fill = uva_color_palette[2]) +
  labs(#title = "Countries where Github users are located", 
...
We say that there is a collaboration between two Github users if they are listed as contributors in one licensesd repository. We say there is a collaboration between two countries if there is a collaboration between two GitHub users who reside in these two countries respectively.    


In our network analysis, 6,745 cities are included, which spanned 165 countries. In the U.S. network, we identified 2820 cities. In the following plot, we are showing the countries that are collaborating with the U.S. Germany, having 453 cities that are collaborating with the U.S., is the major collaborator with the U.S., following by UK, Brazil, and India. 
...
To construct city-level network, we define an edge to be a collaboration between two GitHub users who are located in two different cities. We then define this as a collaboration between two cities. 
...
![](/sfs/qumulo/qhome/zz3hs/git/dspg20oss/src/geographic/output/network1.png) 
</center>
...
<center>
![](/findings/geographic_files/12_all_comtinents_major_cities.png)
</center>
