...
Our city code relies on the geocode provided by the Open Street Maps API, we will further evaluate the validity of the geocode and compare it with the user-input location. We identified users who reside in multiple countries, but we don't have multiple geocodes to reflect both/all of their locations. Currently, we counted the total number of users recide in each city. Futher, we would like to account for the population in each city to reflect the total number of GitHub users per capita.
